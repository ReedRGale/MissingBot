...
async def new_canon(ctx, tb):
    ...
    # Ask for RP name, then formats it.
    tb.reset(val.MEDIUM)
...
async def new_combat(ctx, tb):
    ...
    # Ask for players.
    tb.reset(val.MEDIUM)
    tm = await TidyMessage.build(ctx, get_escape(ctx), content=st.REQ_USER_COMBAT,
                                 checks=[check_member_f(ctx),
...
def comm_ended(tm, ctx):
    return tm.message.embeds[0].description == st.TIMEOUT or tm.prompt.content == get_escape(ctx)
